...
def build_config(args: argparse.Namespace) -> ReinforceConfig:
    ...
    from models.ar_walk_attn import ReinforceConfig
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train AutoregressiveWalkAttn without gold shortest paths."
    )
    parser.add_argument(
        "--actions-path",
...
def main() -> None:
    ...
    if args.checkpoint_output is None:
        args.checkpoint_output = run_dir / "ar_walk_attn.pt"
    if args.metrics_output is None:
        args.metrics_output = run_dir / "train_ar_walk_attn_metrics.json"

    from environment import NamuwikiEnvironment
    from models.ar_walk_attn import (
        AutoregressiveWalkAttn,
