...
from utils import Action, Config, Page, Title
...
class AutoregressiveWalk(Model):
    ...
    def __init__(
        ...
        *,
        ...
        num_layers = int(config.value("num_layers"))
        num_heads = int(config.value("num_heads"))
        dropout = float(config.value("dropout"))
        ...
        self.network = AutoregressivePolicyNetwork(
            embedding_dim=self.embedder.get_embed_dim(),
            num_layers=num_layers,
            num_heads=num_heads,
            dropout=dropout,
            ff_dim=ff_dim,
    ...
    def _resolve_device(self, device: object) -> str:
        ...
        raw_device = "cpu" if device is None else str(device)
        if raw_device != "auto":
            return raw_device
    ...
    def begin_episode(self, start_title: str, target: str) -> None:
        """Reset inference history at the start of an environment rollout."""
    ...
    def _encode_history(self, history: list[str]) -> Tensor:
        ...
        history_embeddings = torch.as_tensor(
            self.embedder.get_embeds(history),
            dtype=torch.float32,
        )
    ...
    def _encode_target(self, target: str) -> Tensor:
        ...
        return torch.as_tensor(
            self.embedder.get_embeds([target]),
            dtype=torch.float32,
            device=self.device,
        )
    ...
    def _encode_actions(self, actions: list[Action]) -> Tensor:
        ...
        return torch.as_tensor(
            self.embedder.get_embeds(actions),
            dtype=torch.float32,
            device=self.device,
        )
    ...
    def sample(self, page: Page, target: str) -> Action | None:
        ...
        if not self._history:
            self._history = [page.title]
        elif self._history[-1] != page.title:
            self._history.append(page.title)

        logits = self.action_logits(self._history, target, list(page.actions))
        best_index = int(torch.argmax(logits).item())
        next_title = page.actions[best_index]
        self._history.append(next_title)
        return next_title
