...
class BeamLexicalSimilarityGreedy(Model):
    """Lexical-similarity greedy baseline with fixed-depth beam-search lookahead."""
    ...
    def score_actions(self, actions: list[str], target: str) -> list[tuple[str, float]]:
        return [(action, cached_similarity(action, target)) for action in actions]
